...
export type OAuthSignInFormProps = JSX.IntrinsicElements['form'] & {
  placeholder?: string
  autoFocus?: boolean
  disabled?: boolean
  loading?: boolean
...
export function OAuthSignInForm({
  ...
  const disabled = props.disabled || props.loading || loading
    ...
    >
      <fieldset className="rounded-md border border-solid border-slate-200 text-neutral-700 dark:border-slate-700 dark:text-neutral-100">
        <div className="relative flex flex-wrap items-center justify-stretch space-x-2 p-1">
          ...
          />
          <Button type="submit" loading={loading || props.loading} transparent>
            Login
          </Button>
        </div>
      </fieldset>
